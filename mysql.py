...
def register(user,pwd,id,email,role,age,gender):
    class re_data():       
        def register(self,user,pwd,id,email,role,age,gender):
            ...
            check_id=cursor.execute(f'''
                                       SELECT USER_ID FROM CREDENTIALS WHERE "{id}" IN (SELECT USER_ID FROM CREDENTIALS);
                                      ''').fetchall()
            check_e=cursor.execute(f'''
                                       SELECT USER_EMAIL FROM CREDENTIALS WHERE "{email}" IN (SELECT USER_EMAIL FROM CREDENTIALS);
                                      ''').fetchall()
            if len(check_e)==0:
                if len(check_id)==0:
                    if len(check_U)==0 :
    ...
    if age>="18":
        ...
        if ob.register(user,pwd,id,email,role,age,gender):
            return True,'Regestration Successfull'
        else:
            message='User Exist!'
...
def up_count(candidate,id):
    ...
    cursor.execute(f'''
                        ...
                        ''')
    cursor.execute(f'''
                        UPDATE VOTERS SET STATUS="Voted" WHERE VOTER_ID="{id}";
                        ''')
...
def auther(username):
    ...
    data = cursor.execute(f''' 
                            select USER_EMAIL from CREDENTIALS WHERE USER_EMAIL== "{username}";
                            ''').fetchall()[0][0]
    if len(data)!=0:
...
def find(user):
    ...
    cursor = connetion.cursor()
    return cursor.execute(f'''
                        SELECT USER_NAME FROM CANDIDATES WHERE USER_ID={user};
                        ''').fetchall()
