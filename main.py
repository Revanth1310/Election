...
def info():
    result = mysql(request.form["user_name"], request.form["pwd"], 'Voter')
    if result:
...
def voting_page():
    ...
    if ud:
        ...
        return render_template('details.html', error='Invalid details!/Voter already Voted' )

@app.route('/vote/<candidate>', methods=['POST', 'GET'])
def thanks(candidate):
    id=request.form['id']
    up_count(candidate,id)
    
    return render_template('thanks.html', candidate=candidate)

@app.route('/result', methods=['POST', 'GET'])
def counting():
    a=find(request.form['candidateid'])
    if a:
        cd=count()
        x = []
        y = []
        ...
        return render_template('details.html',message="Candidate Does Not Exist")
